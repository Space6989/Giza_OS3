...
# Autorzy: Operator Space & Wingman AI (Giza Analysis Squadron)
# ==============================================================================
import os
import time
import threading
...
def update_mission_control(n):
    ...
    log_entries = [
        html.Div(f"{entry}", style={"margin": "2px 0"})
        for entry in list(log_buffer)[::-1]
    ]

    # STEP 2 & 3: SPECTRAL BARS COMPONENT - Dynamically generated with persistence values
    # FIX: Add proper flexbox styling to container for horizontal alignment
    spectral_bars = html.Div([
        html.Div(children=[
            html.Div(style={
                "width": "25px", "height": f"{min(80, e_low*2.5):.0f}px", "backgroundColor": "#00D4FF",
                "borderRadius": "2px", "transition": "height 0.3s"
            }),
            html.Div(f"{sniper.persistence['LOW']}s", style={"fontSize": "10px", "color": "#00D4FF", "textAlign": "center", "marginTop": "5px"})
        ], style={"display": "flex", "flexDirection": "column", "alignItems": "center"}),
        html.Div(children=[
            html.Div(style={
                "width": "25px", "height": f"{min(80, e_mid*2.5):.0f}px", "backgroundColor": "#FFB302",
                "borderRadius": "2px", "transition": "height 0.3s"
            }),
            html.Div(f"{sniper.persistence['MID']}s", style={"fontSize": "10px", "color": "#FFB302", "textAlign": "center", "marginTop": "5px"})
        ], style={"display": "flex", "flexDirection": "column", "alignItems": "center"}),
        html.Div(children=[
            html.Div(style={
                "width": "25px", "height": f"{min(80, e_high*2.5):.0f}px", "backgroundColor": "#FF4444",
                "borderRadius": "2px", "transition": "height 0.3s"
            }),
            html.Div(f"{sniper.persistence['HIGH']}s", style={"fontSize": "10px", "color": "#FF4444", "textAlign": "center", "marginTop": "5px"})
        ], style={"display": "flex", "flexDirection": "column", "alignItems": "center"}),
